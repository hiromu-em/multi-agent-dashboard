...
- **指示はダッシュボード上部のグローバル入力欄から出す。** 以前は「窓口となる別のCLIセッションから `#` 指示を打ち、フックで横取りしてダッシュボードへ渡す」方式だったが、CLIを窓口にする仕様は廃止した。理由は、窓口CLI自身をボードから隠す仕組み（`.logs/gateway.json` への登録）や、配送ループを防ぐためのフック側`session_id`判定など、間接的な仕組みが複数必要になっていたため。今はダッシュボードの `<textarea>` から直接 `POST /api/dispatch` を叩く（`src/app/page.tsx` の `submitDispatch`）。宛先の解決・スティッキー・確認・新規セッション作成は変わらずすべてサーバー側（`src/lib/dispatch.ts` の `routePrompt`）に任せてあるので、フックを挟んでいた頃の判断ロジックはそのまま流用できた
  ...
  **入力欄の高さはボタンで切り替える**（`dispatchExpanded`、既定2行／広げると10行）。長い指示のときだけ広げられればよく、常に大きいとレーンの表示領域を食う。ドラッグでのリサイズ（`resize-y`）は掴みどころが小さく、盤面の他の開閉（ヘッダーなど）がすべてボタンなので、ここも同じボタン式で揃えている

  **送信に失敗したときは入力欄を消さない**（`submitDispatch`、レーンの `ReplyBox` も同じ）。宛先が無い・タグを打ち間違えた、といった失敗はタグを直して送り直すだけなので、本文まで消えると打ち直しになる。消すのは実際に送れた（またはサーバーが本文を預かった）ときだけ
  ...
  **ラベルが重複したら選択肢として扱わない**（`labelsAreUnique`）。`formatAskUserQuestion` は質問が複数あっても選択肢に通し番号を振るので、本物の選択肢はラベルが必ず一意になる。逆に重複しているときは、無関係な番号付きリストが2つ以上混ざった地の文（「1.2.3.」の説明のあとに「1.2.3.」の質問が続く形）なので、選択肢ではない。実際、重複したまま出すと盤面が壊れた——ボタンのReactキーにラベルを使っているため「Encountered two children with the same key, `1`」になる（キー側にも位置を混ぜて二重に防いである）
...
- **ボードのサイズは全レーン共通の固定値**（通常720px幅、フォーカス時980px幅）。高さは画面の残り領域いっぱいに自動で伸びる。縦スクロールは出さない。**ライブモードだけは例外**で、固定幅をやめて画面を等分する（後述）
- **ライブモードは「眺める画面」ではなく「会話するための画面」**（`src/app/page.tsx` の `liveMode`、`LaneCard` の `live`）。余分な表示を外して大きなボードで会話するためのモードで、監視専用ではない。したがって**返信欄は残す**——会話が目的なので主役。外すのはヘッダー、上部のグローバル指示入力欄、各ボードのDiff・拡大・片付け・停止ボタン、作業ディレクトリの行。残すのはタグ・セッション名・状態と、会話と返信欄。会話の文字は15.5px（通常は14px）、返信欄も一回り大きくする

  並べる枚数は1〜3で切り替える（既定3）。1920px幅だと3枚で1枚あたり約600pxとなり通常の720pxより狭くなるが、外したパーツのぶん会話は広く見える。どの3枚を出すかは上の帯のタグ、または◀▶で選ぶ（`liveStart`）。Escで通常モードへ戻る。**フォーカス表示（クリックで980pxにするもの）は残してある**——ライブモードは別物として併存させる
