...
- **ボードの並びは「実行中は左、それ以外は開始時刻の昇順」の2段階**（`src/lib/agents-cli.ts` の `fetchAgents`）。まず開始時刻の昇順で並べ、そのあと「実行中かどうか」だけで安定ソートする。安定ソートなので、実行中どうし・非実行中どうしの前後関係は開始時刻順のまま変わらない。**完了したレーンは、新しく実行中になった別のレーンに追い越されることはあっても、完了したという理由だけで位置が動くことは無い。**
