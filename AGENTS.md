...
**1回目は作らない。同じ宛先へもう一度送られたときだけ作る**（`src/lib/dispatch.ts` の `handleUnknownTag`）。

```
#K 調べておいて     → 「#K という宛先はありません。今ある宛先: #A #B / もう一度 #K へ送ると…」
#K 調べておいて     → #K で新しいセッションを作り、その本文を最初のプロンプトとして渡す
```

即座に自動生成しないのは、`#B` のつもりで `#K` と打ち間違えただけでセッションが生えるのを避けるため。逆に、ただエラーにして終わりだと「じゃあどうやって新しいレーンを作るのか」が窓口から見えない。確認を1回挟む形にして両方を避けている。待ち合わせの状態はプロセス内（`pendingCreate`、5分で失効）に持つ。順番待ちと同じくサーバー再起動で消えてよい類の一時的な状態。
