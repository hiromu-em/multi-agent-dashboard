...
- **ボードの並びは「完了」→「対話待ち」→「実行中」→「エラー・停止済み」の優先度で決まる**（`src/lib/agents-cli.ts` の `fetchAgents`、`STATUS_PRIORITY`）。開始時刻はどこにも使わない（tiebreakとしてのみ残す）。安定ソートを3回チェーンしていて、最後に適用したものが最優先になる：①開始時刻昇順（tiebreak）→②終了時刻（`endedAt`、`lane-registry.ts` が記録）の降順（同じ優先度グループ内では新しく終わったものほど左。実行中・対話待ちは`endedAt`が無い）→③状態グループの優先度。**「完了」と「エラー・停止済み」はグループとして分かれている**（完了は最優先、エラー・停止済みは最後）ので、失敗したレーンが完了と紛れて左側に来ることは無い。同じグループ内では、新しく終わった別のレーンに追い越されることはあっても、時間が経っただけで位置が動くことは無い。
